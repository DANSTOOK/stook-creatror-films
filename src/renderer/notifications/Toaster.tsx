 ...
 *
 * A toast rises into the corner with the bounce; one that goes slides out to
 * the right, and the others close the gap rather than jumping into it.
 */
export function Toaster(): JSX.Element {
  ...
  useFlip(stackRef, shown.map((toast) => toast.id).join('|'));
...
function Toast({ toast }: { toast: AppNotification }): JSX.Element {
  const t = useT();
  const dismiss = useNotifications((state) => state.dismiss);
  const ref = useExitGhost<HTMLDivElement>('toast');
...
export function NotificationsButton(): JSX.Element {
  ...
  return (
    <div ref={rootRef} className="relative">
      ...
      {open && <HistoryPopover history={history} onClear={clearHistory} />}
    </div>
  );
}

/** The bell's list: drops from the button's corner, fades when it closes. */
function HistoryPopover({ history, onClear }: { history: AppNotification[]; onClear(): void }): JSX.Element {
  const t = useT();
  const ref = useExitGhost<HTMLDivElement>('menu');
  return (
    <div
      ref={ref}
      role="dialog"
      aria-label={t('notify.history')}
      className="scf-menu absolute right-0 top-full z-[95] mt-1 w-[380px] origin-top-right rounded-menu border border-panel-600 bg-panel-800 p-1 shadow-2xl shadow-black/60"
    >
      <div className="flex items-center justify-between px-2 py-1.5">
        <span className="text-xs font-semibold text-slate-200">{t('notify.history')}</span>
        {history.length > 0 && (
          <button type="button" className="text-2xs text-slate-400 hover:text-slate-200" onClick={onClear}>
            {t('notify.clear')}
          </button>
        )}
      </div>
