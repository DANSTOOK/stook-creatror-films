...
const exportFile = join(workDir, 'stress-1h.mp4');
const projectFile = join(workDir, 'stress-1h.scf');
const reportFile = join(workDir, 'report.json');

const source = process.env.BENCH_SOURCE ?? '';
const FPS = 30;
const HOUR_FRAMES = 60 * 60 * FPS;
...
async function main() {
  ...
  try {
    ...
    check('17 stills of awkward sizes import into bins', library.stills.length === 17
      && ['Awkward', 'Overlays', 'Slides', 'Stills'].every((name) => library.bins.includes(name)),
    `${library.stills.length} stills, bins [${library.bins.join(', ')}] in ${seconds(report.timings.importStills)}`);
    check('the project adopted the footage', library.project === `1280x720 @ ${FPS}`, library.project);

    /* An hour of edit ------------------------------------------------------------ */
    step('building an hour');
    t = Date.now();
    const built = await window.evaluate(({ hour, seed }) => {
      ...
      // Refill to past the hour, then trim the last clip to end on it exactly.
      while (mainEnd() < hour) put(slides[pickInt(0, slides.length - 1)], mainEnd());
      const last = onMain().at(-1);
      s().trimClip(last.id, 'end', hour);
      ...
      let overlays = 0;
      for (let i = 0; i < 40; i += 1) {
        const start = Math.floor((i + 0.5) * (hour / 40));
      ...
      return { cuts, deleted, graded, overlays, clips: Object.keys(s().project.clips).length };
    }, { hour: HOUR_FRAMES, seed: SEED });
    report.timings.buildHour = Date.now() - t;
    const afterBuild = await window.evaluate(pageInvariants);
    check('an hour of heavy editing keeps every invariant',
    ...
    await dialog.getByLabel('File name').fill('stress-1h');
  ...
  if (!finalProject || !(await stat(exportFile).catch(() => null))) {
    ...
    check('the file is an hour long', info.seconds !== null && Math.abs(info.seconds - 3600) < 0.5,
      `${info.seconds?.toFixed(2)} s, ${(size / 1024 ** 3).toFixed(2)} GB`);
    check('1280x720 H.264 at 30 fps with AAC sound', Boolean(info.video) && info.video[0] === 1280 && info.video[1] === 720
    ...
    check('every one of the 108,000 frames is in the file', frames === HOUR_FRAMES, `${frames} frames (${seconds(Date.now() - t)})`);
    ...
    const covered = (frame) => clips.some((c) => c.trackId === top.id && c.startFrame <= frame && frame < c.startFrame + c.durationFrames);
    ...
    const wrongPictures = [];
    const diffs = [];
    // Both sides sampled MID-frame, at the same container time. A seek that
    // lands exactly on a frame's timestamp can come back with either
    // neighbour, which read as off-by-one in both directions. Container time
    // is also what a player shows: in a file whose first frame is stamped
    // `sourceStart` in, frame N is what is on screen at N/fps.
    for (const sample of pictureSamples) {
      const exported = await frameAt(exportFile, (sample.frame + 0.5) / FPS);
      const expected = await frameAt(source, (sample.sourceFrame + 0.5) / FPS);
      const diff = meanDiff(exported, expected);
      diffs.push(diff);
      if (diff > 12) {
        const earlier = meanDiff(exported, await frameAt(source, (sample.sourceFrame - 1) / FPS));
        const later = meanDiff(exported, await frameAt(source, (sample.sourceFrame + 1) / FPS));
    ...
    const soundSamples = pictureSamples.filter((sample) => {
      const clip = plainFootage.find((c) => c.startFrame <= sample.frame && sample.frame < c.startFrame + c.durationFrames);
      return clip && sample.frame + 3 * FPS < clip.startFrame + clip.durationFrames;
    }).slice(0, 8);
    ...
    check('the sound under the footage is the source sound, in sync with the picture',
      soundSamples.length >= 4 && poorSound.length === 0 && medianR > 0.8,
      poorSound.length
        ? poorSound.slice(0, 3).join('; ')
        : `${soundSamples.length} stretches at ${Math.min(...shifts).toFixed(1)}..${Math.max(...shifts).toFixed(1)} ms (limit one frame, ${(1000 / FPS).toFixed(1)} ms); r ${Math.min(...correlations).toFixed(2)}..${Math.max(...correlations).toFixed(2)}, median ${medianR.toFixed(2)}`);
